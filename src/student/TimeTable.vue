        ...
        <div>
            <div class="container">
                <div class="text-title">
                    <h3>
                        thời khóa biểu lớp {{ Class.nameclass }}
                    </h3>
                </div>
                <table class="table">
...
export default {
    ...
    async created() {
        const id = await JSON.parse(localStorage.getItem('user')).myclassID
        let res = await axios.get(`http://localhost:3000/class/info/${id}`)
        this.Teacher = await res.data.teacher
        this.Class = await res.data
    },
    mounted() {
        this.abc()
    },
    methods: {
        async abc() {
            ...
            const classId = await JSON.parse(localStorage.getItem('user')).myclassID
