<template>
    <div v-if="Teacher.form_teacherID != undefined">
            ...
            <table class="table table-striped">
                ...
                <tbody v-for="(item, index) in Student" :key="index">
                    <tr>
                        ...
                        <td>{{ item.username }}</td>
                        <td><textarea name="" title="Xem nội dung đánh giá" id="" cols="20" placeholder="Chưa có đánh giá"
                                rows="2" style="color: red; cursor: no-drop;"
                                disabled>{{ item.resultID.conduct }}</textarea></td>
                        <td>
                            <button type="button" @click="copyID(item)" title="Nhập vào đánh giá" class="btn btn-primary"
